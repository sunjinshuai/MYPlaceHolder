...
{
    BOOL isEmpty = YES;
    id <UITableViewDataSource> myDataSource = self.dataSource;
    NSInteger sections = 1;
    if ([myDataSource respondsToSelector: @selector(numberOfSectionsInTableView:)]) {
        sections = [myDataSource numberOfSectionsInTableView:self];
    }
    for (int i = 0; i < sections; ++i) {
        NSInteger rows = [myDataSource tableView:self numberOfRowsInSection:i];
        if (rows) {
            isEmpty = NO;
        }
    }
